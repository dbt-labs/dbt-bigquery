...
from dbt.adapters.bigquery.services._config import DEFAULT_TIMEOUT, logger, query_job_url
...
class BigQueryQueryService:
    ...
    def execute(
    ...
    ) -> Tuple[BigQueryAdapterResponse, RowIterator]:
        ...
        return self.response(query_job), iterator

    def response(self, query_job: QueryJob) -> BigQueryAdapterResponse:
        ...
        if query_job.dry_run:
            ...
            code = "CREATE VIEW"
        elif query_job.statement_type in [
            "DELETE",
            "INSERT",
            "MERGE",
            "SCRIPT",
