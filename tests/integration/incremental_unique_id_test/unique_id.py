...
class TestIncrementalUniqueKey(DBTIntegrationTest):
    ...
    def build_test_case( self, seed, incremental_model, update_sql_file, seed_expected_row_count):
        """Make incremental model from seed, then change seed"""
        seeds = self.run_dbt(["seed", "--full-refresh"])
        self.assertEqual(len(seeds), 1)
        
        models = self.run_dbt(["run", "--select", incremental_model, "--full-refresh"])
        ...
        self.assertEqual(seed_expected_row_count, len(self.run_sql(get_row_count, fetch="all")))
    ...
    def run_incremental_mirror_seed_test(self, incremental_model, seed, seed_expected_row_count):
...
class TestIncrementalListUniqueKey(TestIncrementalUniqueKey):
    ...
    @use_profile('bigquery')
    def test__bigquery_unique_key_list_no_update(self):
